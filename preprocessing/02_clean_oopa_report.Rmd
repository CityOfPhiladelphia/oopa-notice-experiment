...
This script cleans the case-level OOPA report data from 2013 to 2017.
...
```{r load text}
raw <- readLines(file.path('..', 'data', 'raw', 'OOPA1317 - v2.txt'))
length(raw)
vis(head(raw))
vis(tail(raw))
```
...
first_header <- which(grepl(
  "TIER 1 PAYMENT DETAIL 20131001 THRU 20171231",
  raw
))[1]
...
# remove page headers
raw <- raw[!grepl(
  "TIER,[1-4],PAYMENT,DETAIL,20131001,THRU,20171231",
  raw)]
```
