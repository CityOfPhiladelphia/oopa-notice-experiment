...
# Blocking variables
block_var <- c('status', 'tier', 'zip_bin', 'different_mail')
```
...
dat %>% 
  filter(!is.na(case)) %>% 
  ggplot(aes(zip_bin)) + geom_bar() + coord_flip()

for(i in block_var[block_var != 'zip_bin']){
  p <- dat %>%
    filter(!is.na(case)) %>%
    ggplot(aes_string(i)) + 
    geom_bar() +
...
ggplot(dat, aes(zip_bin)) + geom_bar() + coord_flip()
